...
def init_db() -> None:
    ...
    with SessionLocal() as session:
        if not session.scalar(select(User).where(User.username == settings.demo_username)):
            session.add(User(username=settings.demo_username, password_hash=hash_password(settings.demo_password)))
            session.commit()
...
def list_history(limit: int = 20) -> list[InvestigationRecord]:
    with SessionLocal() as session:
        rows = session.scalars(select(Investigation).order_by(Investigation.timestamp.desc()).limit(limit)).all()
        return [
            InvestigationRecord(
                id=row.id,
