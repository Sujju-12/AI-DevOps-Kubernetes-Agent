...
def inspect_pods(items: list[dict] | None = None) -> dict:
    """Detect unhealthy pods from `kubectl get pods -o json` items."""
    items = items or []
    problematic = []
    for item in items:
        ...
        unhealthy = phase in {"Pending", "Failed"} or reason in UNHEALTHY_REASONS
