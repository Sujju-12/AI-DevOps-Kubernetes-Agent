  ...
  return (
    <section className="rounded-2xl border border-slate-800 bg-slate-900/70 p-6">
      <div className="mb-4 flex items-center justify-between gap-4">
        ...
        <span className="rounded-full bg-emerald-500/15 px-3 py-1 text-sm text-emerald-300">Confidence {diagnosis.confidence}%</span>
      </div>
      <dl className="space-y-4 text-sm">
        ...
        <div>
          ...
          <dd className="mt-1 overflow-x-auto rounded-lg bg-black/40 p-3 font-mono text-xs text-sky-200">{diagnosis.kubectl_command}</dd>
        </div>
        ...
        <p className="text-xs text-slate-500">Engine: {diagnosis.engine}</p>
      </dl>
    </section>
  );
}
