...
export default function HomePage() {
  ...
  const steps = useMemo(
    () =>
      STEP_DEFS.map((step) => ({
        ...
        active: investigating && activeStep === step.key,
      })),
    [doneSteps, activeStep, investigating]
  );
  ...
  async function runInvestigation() {
    ...
    try {
      ...
      if (result.status === "error") {
        setError(result.error || result.diagnosis?.explanation || "Investigation failed");
      }
  ...
  const healthyEmpty =
    diagnosis && diagnosis.root_cause.toLowerCase().includes("no critical kubernetes issues");
    ...
    <main className="mx-auto min-h-screen max-w-5xl px-6 py-10">
      <header className="mb-8 flex flex-wrap items-center justify-between gap-4">
        <div>
          ...
          <p className="mt-1 text-slate-400">Troubleshoot Kubernetes with AI — fully local</p>
        </div>
      ...
      <div className="space-y-8">
        <ClusterPicker
          contexts={clusters.data?.contexts || []}
          selected={selected}
          onSelect={setSelected}
          warning={clusters.data?.warning}
        />
        ...
        <div>
          ...
          {selected ? (
            <p className="mt-2 text-sm text-slate-400">Selected context: {selected}</p>
          ) : null}
        </div>
        ...
        <section className="rounded-2xl border border-slate-800 bg-slate-900/50 p-6">
          ...
          {investigating ? (
            <p className="mb-4 text-sm text-sky-300">Investigating Kubernetes Cluster...</p>
          ) : null}
          <ProgressList steps={steps} />
        </section>

        {error ? (
          <p className="whitespace-pre-wrap rounded-xl border border-rose-500/30 bg-rose-500/10 p-4 text-sm text-rose-100">
            {error}
          </p>
        ) : null}
        ...
        {diagnosis && !healthyEmpty ? <DiagnosisCard diagnosis={diagnosis} /> : null}
